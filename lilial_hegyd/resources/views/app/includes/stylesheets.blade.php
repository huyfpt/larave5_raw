{{--Global stylesheets for app--}}

{!! Html::style('/vendor/bower/bootstrap/dist/css/bootstrap.min.css') !!}
{!! Html::style('/vendor/bower/jquery-ui/themes/base/jquery-ui.css') !!}

{!! Html::style('/vendor/bower/animate.css/animate.min.css') !!}
{!! Html::style('/vendor/bower/font-awesome/web-fonts-with-css/css/fontawesome-all.min.css') !!}
{!! Html::style('/vendor/bower/metisMenu/dist/metisMenu.min.css') !!}
{!! Html::style('/vendor/bower/jsgrid/dist/jsgrid-theme.min.css') !!}
{!! Html::style('/vendor/bower/switchery/dist/switchery.min.css') !!}
{!! Html::style('/app/vendor/bower/sweetalert/dist/sweetalert.css') !!}


{!! Html::style('/vendor/bower/toastr/toastr.min.css') !!}
{!! Html::style('/app/css/jquery.steps.css') !!}

{!! Html::style('/tmp/css/style.css') !!}
{!! Html::style('/tmp/css/colors/default-dark.css') !!}
{!! Html::style('/tmp/css/add.css') !!}
{!! Html::style('/tmp/css/ecommerce.css') !!}

{!! Html::style('/vendor/hegyd/plans/css/form.css') !!}

<!-- {!! Html::style('/app/css/summernote-ext-resized-data-image.css') !!} -->


@stack('stylesheets')
