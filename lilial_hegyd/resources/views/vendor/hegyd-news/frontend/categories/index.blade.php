...
@php
    $role = app(config('hegyd-news.services.extranet'))->getRole();
@endphp
...
@section('content')
    ...
    <div class="row">
        <div class="menu-admin pull-right">
            @if($can_edit)
                <a class="btn btn-info btn-circle" href="{{ route(config('hegyd-news.routes.backend.news_category.index')) }}"
                   data-toggle="tooltip"
                   title="@lang('hegyd-news::news_categories.button.categories')">
                    <i class="ti-settings"></i>
                </a>
            @endif
            @if($can_add)
                <a class="btn btn-info btn-circle" href="{{ route(config('hegyd-news.routes.backend.news_category.create')) }}"
                   data-toggle="tooltip"
                   title="@lang('hegyd-news::news_categories.button.add')">
                    <i class="ti-plus"></i>
                </a>
            @endif
        </div>
                 ...
                 <div class="col-md-3 col-xs-12 col-sm-4 category-box">
                    <div class="white-box">
                        ...
                        <a class="btn btn-info btn-circle"
                           href="{{ route(config('hegyd-news.routes.backend.news_category.edit'), $category->id) }}"
                           data-toggle="tooltip"
                           title="@lang('hegyd-news::news_categories.button.edit')">
                        ...
                        <a href="{{ $category->url() }}" class="btn btn-info m-t-10 waves-light">@lang('hegyd-news::news_categories.button.discover_news')</a>
                    </div>
                </div>
            @endforeach
        @else
