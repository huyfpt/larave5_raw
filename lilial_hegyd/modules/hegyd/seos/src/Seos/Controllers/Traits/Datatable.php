...
use Hegyd\Seos\Facades\Tools;
use Hegyd\Seos\Models\AbstractModel;
...
trait Datatable
{
    ...
    {
        return view('hegyd-seos::backend.includes.datatable.status')
    ...
    {
        return view('hegyd-seos::backend.includes.datatable.status2')
            ->with('config', $this->_config)
            ->with('field', $field)
            ->with('id', $id)
            ->with('checked', $checked)->render();
    ...
    {
        return view('hegyd-seos::backend.includes.datatable.visible')
            ->with('config', $this->_config)
            ->with('field', $field)
            ->with('id', $id)
            ->with('checked', $checked)->render();
    ...
    {
        return view('hegyd-seos::backend.includes.datatable.status3')
            ->with('config', $this->_config)
            ->with('field', $field)
            ->with('id', $id)
            ->with('checked', $checked)->render();
    ...
    protected function printEnum($field, $enumKey, $id)
    {
        ...
        if (isset($enum[$enumKey]))
        {
            ...
            return view('hegyd-seos::backend.includes.datatable.enum')
    ...
    protected function printImage($field, $value, $id)
    {
        ...
        {
            return view('hegyd-seos::backend.includes.datatable.image')
    ...
    protected function printLink($field, $value, $id)
    {
        ...
        if ($model && isset($field['route']) && isset($field['route_id']))
        {
            ...
            return view('hegyd-seos::backend.includes.datatable.link')
                ->with('route', $route)
                ->with('value', $value)
                ->render();
        }
    ...
    protected function printListRelation($field, $value, $id)
    {
        ...
        if ($model && isset($field['relation']) && isset($field['relation_field']))
        {
            ...
            return view('hegyd-seos::backend.includes.datatable.list')
        ...
        Excel::create('export',
            function ($excel)
            {
                $excel->sheet('test',
                    function ($sheet)
                    {
                        $sheet->loadView('hegyd-seos::backend.contents.export.excel',
                            [
