...
class MailService
{
    public function sendReport($report, $datas)
    {
        ...
        try
        {
            Mail::send('hegyd-plans::emails.report', [
                'report' => $report,
                'reason' => $reason,
                'details'=> $details,
            ], function ($message) use ($email, $reason, $details) {
                $message->subject(trans('hegyd-plans::emails.report.user.subject', ['reason' => $reason]))
                    ->to($email);
            });
    ...
    public function sendReportToAdmin($report, $datas)
    {
        ...
        $emails = app(config('hegyd-plans.repository.users'))->getAdminUsers()->pluck('email');
        ...
        foreach($emails as $email) {
            try
            {
                Mail::send('hegyd-plans::emails.report_to_admin', [
                    'reason' => $reason,
                    'details'=> $details,
                    'report'=> $report
                ], function ($message) use ($email, $reason, $details, $report) {
                    $message->subject(trans('hegyd-plans::emails.report.admin.subject', ['reason' => $reason]))
                        ->to($email);
                });
...
//                    $message->subject(trans('hegyd-plans::emails.report.subject'))
//                        ->to($email);
//                });
//
//            } catch (\Exception $e)
