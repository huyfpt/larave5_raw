<?php namespace Hegyd\Plans\Controllers\Traits;

use Hegyd\Plans\Facades\Tools;
use Hegyd\Plans\Models\AbstractModel;
...
trait Datatable
{
    ...
    protected function buildDatesRangeAndWhere(array &$where, $filterField, $filterFrom, $filterTo)
    {
        if ($filterFrom != null)
        {
            $where[] = [$filterField, '>=', Tools::formatDate($filterFrom, trans('dates.database_format'), trans('dates.php_date_format'))];
        }
        if ($filterTo != null)
        {
            $where[] = [$filterField, '<', Tools::formatDate($filterTo, trans('dates.database_format'), trans('dates.php_date_format'))];
        }
    }
    ...
    {
        return view('hegyd-plans::backend.includes.datatable.status')
    ...
    {
        return view('hegyd-plans::backend.includes.datatable.status2')
            ->with('config', $this->_config)
            ->with('field', $field)
            ->with('id', $id)
            ->with('checked', $checked)->render();
    ...
    {
        return view('hegyd-plans::backend.includes.datatable.visible')
            ->with('config', $this->_config)
            ->with('field', $field)
            ->with('id', $id)
            ->with('checked', $checked)->render();
    ...
    {
        return view('hegyd-plans::backend.includes.datatable.status3')
            ->with('config', $this->_config)
            ->with('field', $field)
            ->with('id', $id)
            ->with('checked', $checked)->render();
    ...
    protected function printDate($field, $param, $id = null)
    {
        ...
        {
            return $param != "0000-00-00 00:00:00" && $param != null ? date(trans('dates.php_datetime_format'), strtotime($param)) : '';
        }
    }
    ...
    protected function printEnum($field, $enumKey, $id)
    {
        ...
        if (isset($enum[$enumKey]))
        {
            ...
            return view('hegyd-plans::backend.includes.datatable.enum')
    ...
    protected function printImage($field, $value, $id)
    {
        ...
        {
            return view('hegyd-plans::backend.includes.datatable.image')
    ...
    protected function printLink($field, $value, $id)
    {
        ...
        if ($model && isset($field['route']) && isset($field['route_id']))
        {
            ...
            return view('hegyd-plans::backend.includes.datatable.link')
                ->with('route', $route)
                ->with('value', $value)
                ->render();
        }
    ...
    protected function printListRelation($field, $value, $id)
    {
        ...
        if ($model && isset($field['relation']) && isset($field['relation_field']))
        {
            ...
            return view('hegyd-plans::backend.includes.datatable.list')
        ...
        Excel::create('export',
            function ($excel)
            {
                $excel->sheet('test',
                    function ($sheet)
                    {
                        $sheet->loadView('hegyd-plans::backend.contents.export.excel',
                            [
