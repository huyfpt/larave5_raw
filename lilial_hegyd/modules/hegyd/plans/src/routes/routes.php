...
], function ()
{
    ...
    Route::get('/plans/address/postcode', 'PlansController@ajaxPostCode');
});

Route::group([
    'namespace' => config('hegyd-plans.controllers.frontend_namespace'),
    'prefix'    => config('hegyd-plans.routes.frontend.prefix_route'),
], function ()
{
    /*
     * Front plans
     */
    Route::get('/plans', [
        'as'   => config('hegyd-plans.routes.frontend.plans.index'),
        'uses' => 'PlansController@index',
    ]);

    Route::get('/plans/{slug}', [
        'as'   => config('hegyd-plans.routes.frontend.plans.show'),
