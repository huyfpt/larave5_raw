...
trait Formable
{
     ...
     * @return boolean
     */
    protected function hasTrait($trait)
    {
        $traits = class_uses_recursive($this);

        return in_array('App\Http\Controllers\Traits\\' . $trait, $traits);
    }
    /*
     * Operations on datas
    ...
    protected function beforeValidate(array $datas)
    {
        ...
        if (isset($datas['start_at']))
        {
            if ($datas['start_at'])
            {
                $datas['start_at'] = date('Y-m-d', strtotime(str_replace('/', '-', $datas['start_at'])));
            } else
            {
                $datas['start_at'] = null;
            }
        ...
        if (isset($datas['end_at']))
        {
            if ($datas['end_at'])
            {
                $datas['end_at'] = date('Y-m-d', strtotime(str_replace('/', '-', $datas['end_at'])));
            } else
            {
                $datas['end_at'] = null;
            }
    ...
    protected function afterSave(Model $model, $data = [])
    {
        ...
        if ($this->hasTrait('Attributable'))
        {
            $model = $this->saveAttributes($data, $model);
        }
